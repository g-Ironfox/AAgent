...
const initialNodes = [
  ...
  { id: 'router-1', type: 'router', name: '任务路由', prompt: '根据用户输入选择最合适的处理分支。', branches: [{ id: 'branch-1', name: '分支 1' }, { id: 'branch-2', name: '分支 2' }], x: 310, y: 238 },
...
export function addNode(type) {
  ...
  const node = type === 'router'
    ? { id: `router-${Date.now()}`, type, name: `Router ${number}`, prompt: '根据输入选择一个分支。', branches: [{ id: `branch-${Date.now()}-1`, name: '分支 1' }, { id: `branch-${Date.now()}-2`, name: '分支 2' }], ...position }
...
export function loadDraft() {
  try {
    ...
    state.nodes = savedNodes.map((node) => node.type === 'router'
      ? { ...node, branches: Array.isArray(node.branches) && node.branches.length ? node.branches : [{ id: `branch-${node.id}-1`, name: '分支 1' }] }
      : node);
