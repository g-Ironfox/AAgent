...
- 最高权限在信息层表现为带 `<Command>` 标记的 `user` 消息，在调度层表现为允许使用 `RPUSH` 插队；
...
| 概念 | 终端中的含义 | 当前实现 |
|------|--------------|----------|
| 信息权限 | LLM 如何理解终端输入 | `terminal` 映射为 `user` 角色的 `<Command>` |
...
```mermaid
flowchart LR
    NORMAL["普通入口事件"] -->|"LPUSH"| Q["main_agent_queue"]
...
终端输入通过 MongoDB 历史进入 callback，再以 `<Command>` 投影到 LLM 上下文。消息不是通过 `active.payload` 穿越调用链。
...
因为 `terminal` 会以带 `<Command>` 标记的 `user` 消息进入 LLM 上下文,且系统 prompt 明确把 Terminal 定义为控制台指令来源,终端接口必须被视为高权限控制面，而不是公开聊天接口。
...
- 事件的信息身份与 `<Command>` callback：[07-事件双重身份与信息回调](07-事件双重身份与信息回调.md)
