...
| `webui_py` | 提供事件流监控;同时作为事件入口,通过 `POST /api/terminal` 提交 `terminal` 事件 | `main.py`, `static/` |
...
flowchart LR
    ...
    L -->|"LPUSH qq 事件"| Q["main_agent_queue<br/>Redis List"]
    WEB["WebUI POST /api/terminal"] -->|"RPUSH terminal 事件"| Q
    Q -->|"BRPOP 弹出"| W["task_worker.py<br/>事件循环"]
    W -->|"record_history"| H["MongoDB<br/>event_history"]
    W --> UI["user_interface<br/>命令 / 授权判断"]
    UI -->|"publish active"| Q
    WEB -->|"terminal 分支"| ACTIVE["生成空 active 事件"]
...
| Key | 类型 | 说明 |
|-----|------|------|
| `main_agent_queue` | List | 任务顺序编排队列,`publish`(LPUSH)/`insert`(RPUSH)双端写入,`BRPOP` 消费 |
