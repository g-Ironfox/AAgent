...
Workflow 使用大于可见视口的内部工作区，当前前端默认尺寸为 `1800px × 1200px`。可见区域作为裁剪视口，不显示水平或垂直滚动条。

用户在画布空白区域按住主键拖动时，通过改变视口的 `scrollLeft` 和 `scrollTop` 平移画布。拖动方向与画布内容移动方向一致：鼠标向右移动时查看更靠左的区域，鼠标向下移动时查看更靠上的区域。

交互边界：

- 仅画布空白区域可以发起平移。
- 从节点上开始拖动时移动节点，不平移画布。
- 从端口上开始拖动时创建或编辑连接，不平移画布。
- 平移只改变视口偏移，不修改任何节点的 `x`、`y` 坐标。
- 平移属于纯界面操作，不触发“未保存”状态。
- 平移过程中光标使用 `grabbing`，空闲时使用 `grab`。

原生滚轮和触控板滚动仍可用于移动视口，但滚动条必须保持隐藏。连接端点定位和拖拽预览计算必须计入视口偏移，确保平移后连线仍与端口对齐。
...
| 数据流 | 已连接输入端 | 兼容输入端 | 输入端跟随拖拽改接到目标输入端，原输出端保持不变 |
...
| 数据流输入 | 有 | 编辑该输入唯一连接，输出端保持不变 | 删除该连接 | 输入端跟随拖拽改接到目标输入端 | 替换目标旧连接后改接，原输出端保持不变 |
...
连线的起点和终点切线方向必须由端口的 `direction` 决定，不能根据两个节点的水平相对位置整体翻转：

- `output` 端始终向节点右侧出线。
- `input` 端始终从节点左侧入线。
- 正式连线和拖拽预览必须使用同一套方向规则。

即使两个节点在水平方向上非常接近、重叠或交换左右位置，连线也必须保持各自端口的出入方向。节点位置只影响控制点距离，不得改变端口切线方向。
...
const startControlX = start.x + curve * startDirection
const endControlX = end.x + curve * endDirection
```
...
“未保存”状态只表示 Workflow 的业务定义发生变化，包括：

- 新增或删除节点。
- 修改节点名称、提示词、模型、工具、分支等配置。
- 创建、删除、替换或改接连接。
- 重置 Workflow。

仅拖动节点、改变节点的画布坐标属于编辑器布局调整，不触发“未保存”提示，也不触发离开页面确认。节点坐标仍保留在内存状态中；用户主动点击保存时，当前布局与节点配置、连接关系一起写入草稿。

选择节点、滚动画布、调整窗口尺寸等纯界面操作同样不得触发“未保存”状态。
