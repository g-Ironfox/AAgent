# 2026-08-01 全项目审查与优化报告

> 日期:2026-08-01。
> 类型:审计报告。
> 性质:历史快照(保留审查当时的分析与方案,不回写)。
> 范围:全部源码(`agent/`、`qqbot/`、`webui/`)、`docker-compose.yml`、Dockerfile、`docs/` 与根目录配置文件。
> 说明:本文回答三个问题:**系统现在的业务逻辑是什么?哪些地方可以精简?哪些地方可以用更巧妙的设计简化?**
> 待办:当前修复状态以[2026-08-02 审查核验汇总](2026-08-02-审查核验汇总.md)为准。
...
| O1 | P4 | `agent`/`qqbot` 容器曾使用 `sleep infinity` | **已修复**:`docker-compose.yml` 已直接启动入口脚本 | 7.1 |
| O2 | P4 | `mongodb` 曾无健康检查 | **已修复**:已增加 healthcheck 并使用 `service_healthy` | 7.2 |
...
**方案 B(已采用,2026-08-01)**:干脆不用 `tool` 角色——详见[格式规范](../../06-格式规范.md),代码在 `llm.py::chat_with_deepseek()`:
...
> 当前状态:`docker-compose.yml` 已通过显式 `command` 运行 `python task_worker.py` 与 `python listener.py`;开发模式才通过 `docker-compose.dev.yml` 覆盖为 `sleep infinity` 以便手动调试。两个 Dockerfile 本身仍未声明 `CMD`。

审查时发现 `agent` 与 `qqbot` 只执行 `sleep infinity`,导致 `compose up` 后业务进程未启动。该问题现已通过生产 Compose 的启动命令解决,以下内容保留为决策记录。
...
### 7.2 O2:MongoDB 健康检查(已修复)

审查时 `webui` 对 MongoDB 只使用 `service_started`,存在启动竞态。当前 Compose 已加入鉴权 `mongosh ping` healthcheck,并让 `agent`、`webui` 等待 `service_healthy`;以下示例仅保留方案背景:
...
| 阶段 | 内容 | 收益 | 风险 |
|------|------|------|------|
| **第一阶段:止血(正确性)** | C1 已实施(见[格式规范](../../06-格式规范.md));C2 `_id` 排序、C3 arguments 容错仍待实施 | 消灭上下文顺序错误与工具链断裂 | 低,纯局部改动 |
...
2. **正确性风险仍需优先处理**:C1 已通过文本投影解决;C2 历史排序与 C3 工具参数容错尚未落地;
3. **精简空间明确**:约 150 行重复/死代码 + 3 个未使用依赖 + 命名空间污染,都是低风险清理;
4. **更巧妙的设计**集中在四个"注册化"重构(工具/事件/命令/消息),让新增能力从"改多处"变成"注册一处";
5. **部署基础问题已收口**:生产 Compose 已直接启动业务进程并等待依赖健康;剩余部署风险是可选 Key 强制读取与 Redis/MongoDB 权限加固。
