...
- 终端提交的仍是普通 `terminal` 事件，不绕过 Redis、Worker、MongoDB 或事件分发；
- 最高权限在信息层表现为带 `role='admin'` 标记的 `user` 消息（`<Msg role='admin'>`），在调度层表现为允许使用 `RPUSH` 插队；
- 终端页是全局终端的历史投影，只展示 MongoDB 中已经消费的 `terminal` 和 `response`；
- 终端按时间追加事件，不推断某条 `response` 属于哪条 `terminal` 命令。

## 1. 设计起点：最高权限人工入口

AAgent 只有一个决策中心、一份全局认知和一条连续时间线。终端不是面向普通用户的聊天会话，而是可信操作者向这个决策中心直接提交管理意图的人工控制面。

这里的“最高权限”首先是**信息语义上的权限**。`task_worker.py` 将 `terminal` 事件投影为：

```python
{"role": "user", "content": "<Msg role='admin'>...</Msg>"}
```
...
最高权限不等于特殊事件模型。终端提交的 `terminal` 与其他事件一样，仍然遵守：
...
| 概念 | 终端中的含义 | 当前实现 |
|------|--------------|----------|
| 信息权限 | LLM 如何理解终端输入 | `terminal` 映射为 `user` 角色的 `<Msg role='admin'>` |
| 调度优先级 | 事件在 Redis List 中何时被消费 | `POST /api/terminal` 使用 `RPUSH`，靠近 `BRPOP` 消费端 |
...
flowchart LR
    ...
    TERMINAL["终端 terminal 事件"] -->|"RPUSH 插队"| Q
    INTERNAL["内部连续性事件"] -->|"RPUSH 插队"| Q
    Q -->|"BRPOP"| WORKER["单 Worker"]
    WORKER --> HISTORY["MongoDB 历史"]
```

`RPUSH` 不会改变事件的类型或处理管线。Worker 弹出 `terminal` 后仍先调用 `record_history()`，再生成空 `active` 事件触发后续推理。

### 3.1 当前优先级的边界

Redis List 只有两端，没有真正的多级优先级。终端 `terminal` 与内部 `active`、`response`、`tool`、`tool_return` 都可能使用 `RPUSH`，所以它们共享同一个高优先级端。
...
终端服务的是唯一决策中心，不服务某个隔离会话。因此页面展示全局的 `terminal` 和 `response` 历史：

- `terminal` 表示管理员从终端提交的命令；
...
```text
Worker BRPOP terminal
  → record_history(terminal)
  → 发布空 active
  → active 调用 LLM
  → response 进入事件流
  → Worker 消费 response
...
终端输入通过 MongoDB 历史进入 callback，再以 `<Msg role='admin'>` 投影到 LLM 上下文。消息不是通过 `active.payload` 穿越调用链。

### 5.3 读路径

```text
浏览器轮询 GET /api/terminal/history?limit=150
...
`GET /api/terminal/history` 接受 `limit`，默认 `150`，范围为 `1..300`。
...
终端与事件页共用同一种导航栏控制：刷新间隔、暂停、立即刷新、最后同步时间和自动刷新状态。
...
4. 新 `terminal` 渲染为右侧 `TERMINAL` 命令；
...
因为 `terminal` 会以带 `role='admin'` 标记的 `user` 消息进入 LLM 上下文，终端接口必须被视为高权限控制面，而不是公开聊天接口。

当前 FastAPI 路由本身没有认证、授权或 CSRF 防护，Compose 还可能把端口直接发布到宿主机。因此部署必须至少保证：

- WebUI 仅暴露在可信网络或本机；
- 对外部署时由反向代理提供强认证和访问控制；
- 不允许不可信用户调用 `POST /api/terminal`；
...
4. 终端只展示已提交历史，不把“已入队”伪装成“已执行”；
5. 页面不创建会话隔离，不推断未经协议证明的事件关联；
6. 终端的管理员语义必须与访问控制强度匹配。

### 9.2 当前非目标

- 把 `terminal` 与 `response` 配成问答对；
...
- 事件的信息身份与 `<Msg role='admin'>` callback：[07-事件双重身份与信息回调](07-事件双重身份与信息回调.md)
