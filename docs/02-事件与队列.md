...
- 普通外部事件使用 `LPUSH` 排队，最高权限终端的 `webui` 事件使用 `RPUSH` 插队;
...
| `webui` | WebUI `POST /api/terminal` | `RPUSH` | 记录历史 → 生成空 `active` 事件 `publish` 触发 LLM |
| `active` | `user_interface()` / `handle_task` 的 webui 分支 / `handle_task` 的 active 分支(有 tool_calls 时) | 前者 `publish`,后两者 `insert` | 记录历史 → `chat_with_deepseek()` → 生成 `response` 事件 |
...
5. 新事件不能抢占正在执行的调用；QQ 普通排队，最高权限终端的 WebUI 事件可影响当前事件完成后的下一次出队顺序;
6. 禁止为 `active` 增加线程池、协程任务、fire-and-forget 或额外消费者。

这些不变量的设计依据及上下文脑裂模型见[设计哲学](00-设计哲学.md#为什么必须串行)。

所有事件在 worker 中先通过 `record_history()` 持久化,再执行分发逻辑。统一记录不等于所有事件都会直接变成 LLM 消息:当前 `llm.py` 将 `qq`、`webui`、`response` 和 `tool_return` 映射到 prompt,而 `active`、`tool` 主要承担调度、执行与追踪职责。`response` 与 `tool_return` 因而既是工具链控制流中的事件,也是 LLM 上下文、WebUI 监控和未来 document 召回的记录材料。
...
stateDiagram-v2
    [*] --> qq: qqbot 收到消息 (LPUSH)
    [*] --> webui: WebUI POST /api/terminal (RPUSH 插队)
    qq --> active: 授权用户 且 (私聊/被@) (LPUSH)
    qq --> [*]: /命令直接回复
    webui --> active: 生成空 active 事件 (LPUSH)
...
设计意图:普通外部消息**排队**,内部连续性事件和最高权限终端命令**插队**。终端仍提交普通 `webui` 事件并经过完整 Worker 管线，只是拥有插队资格；详见 [08-终端设计](08-终端设计.md)。List 同时吸收入口突发流量。
...
```
[POST /api/terminal] → WebUI 封装 → RPUSH webui {message: "..."}
              ↓
agent BRPOP 弹出 webui → record_history → LPUSH active {}
              ↓
agent BRPOP 弹出 active → chat_with_deepseek → callback 历史(含 webui 消息)
              ↓
（后续工具链与 QQ 入口完全相同）
```

> 注意:WebUI 的 `active` 事件不携带 `user_id`/`group_id`/`message`。消息内容在上一步 `webui` 事件中已写入历史,LLM 通过 `get_recent_history()` 自行 callback。详见 [07-事件双重身份与信息回调](07-事件双重身份与信息回调.md)。
