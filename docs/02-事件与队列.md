...
| `web` | WebUI `POST /api/chat` | `publish`(LPUSH) | 记录历史 → `user_interface()`(天然授权,payload 带 `source: "web"`) |
| `active` | `user_interface()` / `llm.py`(有 tool_calls 时) | 前者 `publish`,后者 `insert` | 记录历史 → `chat_with_deepseek()` → 发送回复(QQ 外呼 / web 写 `web_reply`) |
...
| `web_reply` | `task_worker.py` active 分支(source=web) | `insert` | 仅记录历史,供 Web 聊天页读取展示 |
